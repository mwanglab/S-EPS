...
sub_detection_rank <- function(selected_region, list_sub) {
  ...
  for (k in 1:length(list_sub)) {
    ...
    if (length(pot_source_k > 1)) {
      p1 <- list_sub_summary[k, (length(selected_region)+1) : (length(selected_region) * 2)]
      source_k <- pot_source_k[which.max(p1[pot_source_k])]
    } else {
      source_k <- pot_source_k
    }
    
    list_sub_summary[k,length(selected_region)*3 + source_k] <- 1
    
    #Step 2.2: Occurrence ranking of the remaining regions
    #Use the second stage sampling data (sample size = 163) to determine the occurrence ranking of the remaining regions for each key mutation.
    ...
    for (l in setdiff(1:length(selected_region), source_k)) {
      if ((!is.na(detection_rank2[l]))&
          ...
          (sample_size[list_sub_summary[k,l + length(selected_region)*2]-1,l] < sample_size_second_stage)) {
